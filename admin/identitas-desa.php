	    			...
	    			<div class="box-body">
	    				...
	    				<form action="" method="POST" enctype="multipart/form-data">
	    				    ...
	    				    <div class="form-group">
	    					     <label>Nama</label>
	    					     <input type="text" name="nama" placeholder="Nama Desa" value="<?= $d->nama ?>" class="input-control" required>
                            </div>

                             <div class="form-group">
	    					     <label>Email</label>
	    					     <input type="email" name="email" placeholder="Email Desa" value="<?= $d->email?>" class="input-control" required>
                            </div>

                             <div class="form-group">
	    					     <label>Telepon</label>
	    					     <input type="text" name="telp" placeholder="Telepon Desa" value="<?= $d->telepon ?>" class="input-control" required>
                            </div>
                            ...
                            <div class="form-group">
	    					     <label>Favicon</label>
	    					     <img src="../uploads/identitas/<?= $d->favicon ?>" class="image">
	    				...
	    				<?php
	    				    ...
	    				    if(isset($_POST['submit'])){
	    				    	...
	    				    	if($_FILES['logo_baru']['name'] != ''){
		    				    	...
		    				    	$allowedtyp_logo = array('png','jpg','jpeg','gif');


	    				    	    if(!in_array($formatfile_logo, $allowedtype_logo)){
	    				    		
		    				    	    echo '<div class="alert alert-error">Format file logo desa tidak diizinkan.</div>';

		    				    	    return false;

		    				    	}elseif($filesize_logo > 1000000){

		    				    		echo '<div class="alert alert-error">Ukuran file logo desa tidak boleh lebih dari 1 MB.</div>';
	    				    	...
	    				    	if($_FILES['favicon_baru']['name'] != ''){
	    				    	    ...
	    				    	    if(!in_array($formatfile_favicon,$allowedtype_favicon)){
		    				    	    ...
		    				    	    echo '<div class="alert alert-error">Format file favicon desa tidak diizinkan.</div>';

		    				    	    return false;

		    				    	}elseif($filesize_favicon > 1000000){

		    				    		echo '<div class="alert alert-error">Ukuran file favicon desa tidak boleh lebih dari 1 MB.</div>';
	    				        ...
	    				        if($update){
	    				              echo "<script>window.location='identitas-desa.php?success=Edit Data Berhasil'</script>";
	    				        }else{
